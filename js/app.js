/**
 * VortexPay — веб-версия консольного тренажёра криптотрейдинга.
 * Реальные рыночные данные берутся с публичного API Binance,
 * покупка/продажа полностью виртуальны.
 */
...
function init() {
  ...
  if (saved) {
    ...
    setStatus(`Загружен сохранённый прогресс: ${state.symbol}, баланс ${fmt(state.portfolio.balanceUsdt)} USDT`);
  } else {
    state.portfolio = new Portfolio(CONFIG.initialBalance);
    setStatus(`Новый тренировочный портфель создан. Стартовый баланс ${fmt(CONFIG.initialBalance)} USDT`);
  }
...
async function loadMarketData() {
  try {
    ...
    renderAll();
    setStatus(`Обновлено ${state.lastUpdate.toLocaleTimeString("ru-RU")} · ${state.symbol}`);
  } catch (err) {
    setStatus(err instanceof ApiError ? err.message : "Не удалось загрузить данные с Binance", true);
  }
}
...
function renderPortfolio() {
  ...
  if (!openPositions.length) {
    positionsBody.innerHTML = '<tr><td colspan="5" class="empty-row">Открытых позиций нет</td></tr>';
  } else {
...
function renderHistory() {
  ...
  if (!trades.length) {
    list.innerHTML = '<div class="empty-row">Сделок пока не было</div>';
    return;
  }
  ...
  for (const t of trades) {
    ...
    const time = new Date(t.timestamp).toLocaleString("ru-RU");
    const sideLabel = t.side === "BUY" ? "Покупка" : "Продажа";
    row.innerHTML = `
...
function bindEvents() {
  ...
  el("symbol-form").addEventListener("submit", async (e) => {
    ...
    setStatus(`Проверка пары ${raw}…`);
    try {
      await getPrice(raw);
    } catch {
      setStatus(`Пара ${raw} не найдена на Binance`, true);
      return;
    }
    state.symbol = raw;
    persist();
  ...
  el("buy-form").addEventListener("submit", (e) => {
    e.preventDefault();
    if (state.currentPrice == null) { setStatus("Цена ещё не загружена", true); return; }
    ...
    try {
      ...
      setStatus(`Куплено ${trade.amount.toFixed(6)} ${baseAsset(state.symbol)} по ${fmt(trade.price)} USDT`);
      el("buy-amount").value = "";
      persist();
      renderAll();
    } catch (err) {
  ...
  el("sell-form").addEventListener("submit", (e) => {
    ...
    try {
      ...
      setStatus(`Продано ${trade.amount.toFixed(6)} ${baseAsset(state.symbol)} по ${fmt(trade.price)} USDT`);
  ...
  el("reset-button").addEventListener("click", () => {
    if (!confirm("Сбросить прогресс и начать заново? Это действие необратимо.")) return;
    clearState();
    state.portfolio = new Portfolio(CONFIG.initialBalance);
    persist();
    renderAll();
    setStatus("Прогресс сброшен. Новый тренировочный портфель создан.");
  });
}
